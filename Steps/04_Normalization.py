...
def main(args):
    ...
    log = outdir / f"{prefix}_step04_ConcatenateNormalize.log"
    logging.basicConfig(level=logging.INFO, handlers=[
        logging.FileHandler(str(log)),
        logging.StreamHandler()
    ], format="%(asctime)s %(levelname)s: %(message)s"

    logging.info("Started step 4: Normalization")
    ...
    logging.info(f"Loading up to {len(samples)} h5ad files and concatenating...")
    adatas = []
    loaded_samples = []
    for s in samples:
        in_path = Path(args.file_path.format(sample=s))
        if not in_path.exists():
            logging.warning(f"Missing h5ad for sample {s}; skipping {in_path}")
            continue

        logging.info(f"reading {s}: {in_path}")
    ...
    logging.info(f"Result: {adata.n_obs} cells, {adata.n_vars} genes")

    raw_out = outdir / f"{args.output_prefix}_raw_concatenated.h5ad"
    logging.info(f"Writing raw concatenated output: {raw_out}")
    adata.write(raw_out)
    
    # Pre-normalization histogram (Using the clean upstream total_counts)
    logging.info("Writing pre-normalization histogram...")
    ...
    logging.info("Running scanpy: normalize_total (per cell) ...")
    scales_counts = sc.pp.normalize_total(adata, target_sum=None, inplace=False)

    logging.info("Running scanpy: log1p ...")
    # log1p transform
    adata.layers["log1p_norm"] = sc.pp.log1p(scales_counts["X"], copy=True)

    # Post-normalization histogram (Shifted Logarithm Profile)
    logging.info("Writing post-normalization histogram...")
    ...
    plt.savefig(plot_path)
    plt.close()
    logging.info(f"Wrote post-normalization histogram: {plot_path}")

    out_h5 = outdir / f"{args.output_prefix}_normalized_concatenated.h5ad"
    logging.info(f"Writing output: {out_h5}")
    adata.write(out_h5)
    logging.info("Done.")
